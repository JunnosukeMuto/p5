...
void draw() {
  ...
  // 配列x[], y[] を使用してデータを描画
  if (drawOK) {
    MyDrawGraph(x, y1, x[0], 0, x[graphN-1], 40, 40, 140, 600, 40, graphN/nlx, 0, 40, 10*scaleY);
    MyDrawGraph(x, y2, x[0], 20, x[graphN-1], 80, 40, 300, 600, 160, graphN/nlx, 20, 80, 20*scaleY);
    MyDrawGraph(x, y3, x[0], 960, x[graphN-1], 1050, 40, 440, 600, 320, graphN/nlx, 960, 1040, 20*scaleY);
  }
}
...
void MyDrawGraph(int [] xdata, float [] ydata, 
  ...
  int ly1, int ly2, float ldy  // y軸ラベルの始点，終点，増分
  ) {
  float x0, y0, x1, y1;
  // 温度軸（Y軸）目盛り
